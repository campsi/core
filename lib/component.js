...
var componentInstanciationCount = 0;

Component.prototype = Object.create({

    // Static Methods

    getDefaultValue: new Function(),
    ...
    getHtml: function (data, options, callback) {
        callback('<pre data-type="' + this.type + '">' + JSON.stringify(data) + '</pre>');
    },
    ...
    getAdvancedFormOptions: function (context) {
        return {
            fields: []
        }
    },
    ...
    init: function (next) {
        ...
        this.id = safeType + '-' + componentInstanciationCount++;
    ...
    wakeUp: function (el, context, callback) {
        ...
        Object.keys(nodePaths).forEach(function wakeUpNode(name) {
    ...
    setValue: function (value, callback, propagateChangeEvent) {
        ...
        if (equals(this.value, value) && this.valueHasBeenSet) {
        ...
        instance.processValue(value, function valueProcessed(err, processed) {
            err = err || [];
            instance.setErrors(err, function errorsHaveBeenSet() {
                ...
                instance.valueDidChange(function valueDidChangeCallback() {
                    cb();
    ...
    setErrors: function (errors, callback, propagateErrorEvent) {
        ...
        var cb = callback || noop;
        var instance = this;
        this.errorsDidChange(function errorsDidChangeCallback() {
    ...
    bind: function (eventName, cb) {
        ...
        if (this._events[eventName].indexOf(cb) === -1) {
            this._events[eventName].push(cb);
        } else {
            console.warn("callback", cb, "for event", eventName, "already attached to comp", this);
        }
    },
